...
export class PlazasList implements OnInit, OnDestroy {
  ...
  readonly plazas = signal<PlazaPublica[]>([]);
  ...
  obtenerCoincidenciaSecundaria(plaza: PlazaPublica): { etiqueta: string; texto: string } | null {
    const busqueda = this.normalizarTexto(this.search().trim());
    ...
    if (
      this.normalizarTexto(plaza.nombre).includes(busqueda) ||
      this.normalizarTexto(plaza.ubicacion).includes(busqueda)
    ) {
      return null;
    }
